...
import type { Employee } from '@/stores/gate/employee/type';
import type { StringOpt } from "@/app/type";

const route = useRoute();
// const employeeStore = useEmployeeStore();
// const employeePhotoUrl = ref(null); 
  
// onMounted(async () => {
//     const employeeId = route.params.id;
//     await employeeStore.setEmployeeById(employeeId);

//     if (employeeStore.loadEmployee.photo) {
//         employeePhotoUrl.value = await fetchPhotoUrl(employeeStore.loadEmployee.photo);
//     }
// });
  
// const employee = computed(() => employeeStore.loadEmployee);
...
//   delete formData.value.attendanceUserId;
//   delete formData.value.attendanceUserLabel;
// //   delete formData.value.id;
//   delete formData.value.createdAt;
//   delete formData.value.updatedAt;
//   delete formData.value.creator;
//   delete formData.value.creator;
//   delete formData.value.location;
//   delete formData.value.dialCode;
//   delete formData.value.village;
//   delete formData.value.updater;
//   await employeeStore.updateEmployee(formData.value); // Pass the updated employee data
// };
...
//     ...formData.value,
//   };
//   await employeeStore.updateEmployee(updatedEmployeeData);
// };
const employeeStore = useEmployeeStore();
// Reactive references
...
// Photo URL fetching method
const loadEmployeePhoto = async (photoFilename) => {
  if (photoFilename) {
    try {
      employeePhotoUrl.value = await employeeStore.fetchPhotoUrl(photoFilename);
    } catch (err) {
      console.error('Failed to load employee photo:', err);
      employeePhotoUrl.value = null;
    }
  }
};
...
const fetchEmployeeData = async () => {
  ...
  try {
    ...
    await employeeStore.setEmployeeById(employeeId);
    // Update employee ref
    employee.value = employeeStore.loadEmployee;
    ...
    if (employee.value) {
      formData.value = {
        ...employee.value,
        dialCode: {
          value: employee.value.dialCode || '',
          label: employee.value.phone || ''
        },
        village: {
          label: employee.value.location?.village?.label || '',
          value: employee.value.location?.vilage?.value || ''
        },
        photo: employee.value.photo || null
      };

      // Load employee photo
      await loadEmployeePhoto(employee.value.photo);
    }
  } catch (err) {
...
// Watch route changes
watch(() => route.params.id, fetchEmployeeData, { immediate: true });
...
const handleSubmit = async () => {
  try {
    ...
    const updatedEmployee = await employeeStore.updateEmployee({
      id,
      ...submitData
    });
...
const handlePhotoUpload = async (event) => {
  ...
  if (file) {
    try {
      ...
      const uploadedPhotoUrl = await uploadPhoto(file);
      
      // Update form data and photo URL
      formData.value.photo = file.name;
      employeePhotoUrl.value = uploadedPhotoUrl;
    } catch (error) {
...
onMounted(() => {
  if (employeePhotoUrl.value) {
    URL.revokeObjectURL(employeePhotoUrl.value);
  }
});
</script>
                ...
                <form @submit.prevent="handleSubmit">
                  ...
                  <div class="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
                    <div class="w-full sm:w-1/2">
                      ...
                      <div class="relative">
                        ...
                        <input v-model="formData.name" class="w-full rounded border border-stroke bg-gray py-3 pl-11.5 pr-4.5 font-normal text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary" type="text" name="fullName" id="fullName" placeholder="Devid Jhon" />
                      </div>
                    </div>
                    <!-- Phone Number Section -->
                    <div class="w-full sm:w-1/2">
                      <label class="mb-3 block text-sm font-medium text-black dark:text-white" for="phoneNumber">Phone Number</label>
                      <input v-model="formData.phone" class="w-full rounded border border-stroke bg-gray py-3 px-4.5 font-normal text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary" type="text" name="phoneNumber" id="phoneNumber" placeholder="+990 3343 7865" />
                    </div>
                  </div>
                  ...
                  <div class="mb-5.5">
                    ...
                    <div class="relative">
                      ...
                      <input v-model="formData.email" class="w-full rounded border border-stroke bg-gray py-3 pl-11.5 pr-4.5 font-normal text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary" type="email" name="emailAddress" id="emailAddress" placeholder="devidjond45@gmail.com" />
                    </div>
                  </div>
                  <!-- Username Section -->
                  <div class="mb-5.5">
                    <label class="mb-3 block text-sm font-medium text-black dark:text-white" for="Username">Username</label>
                    <input v-model="formData.name" class="w-full rounded border border-stroke bg-gray py-3 px-4.5 font-normal text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary" type="text" name="Username" id="Username" placeholder="devidjhon24" />
                  </div>
                  <!-- Bio Section -->
                  <div class="mb-5.5">
                    ...
                    <div class="relative">
                      ...
                      <textarea v-model="formData.bio" class="w-full rounded border border-stroke bg-gray py-3 pl-11.5 pr-4.5 font-normal text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary" name="bio" id="bio" rows="6" placeholder="Write your bio here"></textarea>
                    </div>
                  </div>
                  <!-- Save and Cancel Buttons -->
                  <div class="flex justify-end gap-4.5">
                    <button class="flex justify-center rounded border border-stroke py-2 px-6 font-medium text-black hover:shadow-1 dark:border-strokedark dark:text-white" type="button" @click="handleCancel"> Cancel </button>
                    <button class="flex justify-center rounded bg-primary py-2 px-6 font-medium text-gray hover:bg-opacity-90" type="submit"> Save </button>
                  </div>
                </form>
              </div>
            </div>
                ...
                <form @submit.prevent="handlePhotoSubmit">
                  ...
                  <div class="mb-4 flex items-center gap-3">
                    <div class="h-14 w-14 rounded-full">
                        <img :src="employeePhotoUrl" alt="User" v-if="employeePhotoUrl" />
                    </div>
                    <div>
                  ...
                  <div class="flex justify-end gap-4.5">
                    <button class="flex justify-center rounded border border-stroke py-2 px-6 font-medium text-black hover:shadow-1 dark:border-strokedark dark:text-white" type="button" @click="handlePhotoCancel"> Cancel </button>
                    <button class="flex justify-center rounded bg-primary py-2 px-6 font-medium text-gray hover:bg-opacity-90" type="submit"> Save </button>
                  </div>
                </form>
              </div>
