...
// import useErrors from 'src/use/useErrors'
...
const login = async () => {
	const success = await authStore.login(form.userUnique, form.password);
  ...
  <DefaultLayout>
    ...
    <DefaultAuthCard subtitle="Start for free" title="Sign In to TailAdmin">
      <form @submit.prevent="login">
        <InputGroup v-model="form.userUnique" label="Email" placeholder="Enter your user">
          <svg
            class="fill-current"
            width="22"
            height="22"
        ...
        <InputGroup v-model="form.password" label="Password" type="password" placeholder="6+ Characters, 1 Capital letter">
          <svg
            class="fill-current"
            width="22"
            height="22"
